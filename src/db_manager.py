...
class DBManager(DBClass):
    ...
    def get_companies_and_vacancies_count() -> list[dict[str, Any]]:
        ...
        with self.conn.cursor() as cur:
            cur.execute("""
                ...
                left join (select count(vacansy_id) count_vac, employer_id from vacansies order by employer_id) 
                vacansies on Using(employer_id)
            """)
    ...
    def get_vacancies_with_keyword(list_words: list) -> list[dict[str, Any]]:
